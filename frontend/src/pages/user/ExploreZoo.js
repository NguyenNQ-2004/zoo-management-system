    ...
    <div id="explore" style={{ padding: '80px 60px', maxWidth: '1400px', margin: '0 auto', backgroundColor: '#f9fafb' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '15px' }}>
          ...
          <div style={{ position: 'relative' }}>
            <select style={{ padding: '12px 40px 12px 15px', border: '1px solid #e5e7eb', borderRadius: '8px', appearance: 'none', background: 'white', minWidth: '150px', outline: 'none' }}>
        ...
        {exhibits.map(ex => (
          <div key={ex.id} className="exhibit-card">
            ...
            <div className="exhibit-content">
              ...
              {ex.status === 'Open' ? (
                <button className="btn-primary btn-block">Explore Exhibit <ArrowRight size={16} /></button>
              ) : (
                <button className="btn-secondary btn-block btn-disabled" disabled>Exhibit Details</button>
              )}
            </div>
          </div>
        ))}
